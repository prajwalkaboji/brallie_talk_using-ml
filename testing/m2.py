...
MODEL_PATH = r"E:\bml\braille_character_model.h5"
IMG_SIZE = (28, 28)
EPOCHS = 10
BATCH_SIZE = 32
...
for filename in os.listdir(DATASET_DIR):
    if filename.lower().endswith((".png", ".jpg", ".jpeg")):
        label_char = filename[0].upper()  # first letter of filename is label
...
X = np.expand_dims(X, axis=-1)  # shape: (n_samples, 28, 28, 1)
y = np.array(y)
...
# ============================
model = Sequential([
    Conv2D(6, (5,5), activation='tanh', input_shape=(IMG_SIZE[0], IMG_SIZE[1], 1), padding='same'),
