...
MODEL_NAME = r"D:\bml\vgg16_braille_numbers1.keras"
...
def load_dataset(path=DATASET_PATH, img_size=64):
    data, labels = [], []
    # ✅ Only classes you want (1–4 in your case)
...
# 2. Build VGG16 Model
# -------------------------------
def build_vgg16(input_shape=(IMG_SIZE, IMG_SIZE, 3), num_classes=4):
...
def realtime_prediction():
    ...
    _, _, classes = load_dataset(DATASET_PATH, IMG_SIZE)  # get class names
    ...
    while True:
        ...
        # Draw ROI box (fixed region for hand/number)
        ...
        conf = np.max(pred)
        label = classes[label_idx]  # use class name
